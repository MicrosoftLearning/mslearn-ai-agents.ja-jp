...
AI Toolkit の拡張機能を既にインストールしている場合は、このセクションをスキップできます。
...
1. **[マイ リソース]** の **[Microsoft Foundry]** の下にある 3 つの水平バー アイコンを選択して、既定のプロジェクトを選択します。

    既定のプロジェクトが既にアクティブな場合、プロジェクト名が **[Microsoft Foundry]** セクションの横に表示されます。 別のプロジェクトを選択するには、同じ **[プロジェクトの選択]** アイコンを選択します。

2. **[Microsoft Foundry]** セクションを展開します。 **[エージェント]** の下に、ポータルで作成した `it-support-agent` が表示されるはずです。 エージェント名を選択して、エージェント ビルダー インターフェイスを開きます。
...
1. `.env` ファイルで、`your_project_endpoint_here` を実際のプロジェクト エンドポイントに置き換えます。
    ...
    **プロジェクト エンドポイントを取得するには:** VS Code で、**[AI Toolkit]** 拡張機能を開き、アクティブなプロジェクトを右クリックし、**[エンドポイントのコピー]** を選択します。
