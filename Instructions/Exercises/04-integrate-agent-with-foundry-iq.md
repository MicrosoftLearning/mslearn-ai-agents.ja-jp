...
1. ファイルがアップロードされたら、Foundry ポータルで作成した検索サービスに移動します。
...
1. **［作成］** を選択します
1. ナレッジ ベース作成ページで、**[チャット入力候補モデル]** ドロップダウンから `gpt-4.1` モデルを選択し、残りのフィールドは既定値のままにします。
...
1. **[接続されたリソース]** を下スクロールすると、検索サービスが表示されるはずです。 その行を選択し、**[認証]** セクションを見つけて **[認証の編集]** を選択します。
...
1. **[ツール]** セクションで、**Foundry IQ** (ナレッジ ベース) ツールを見つけ、その 3 つのドット (**...**) を選択してツール構成ポップアップを開きます。

    > **注**: エージェントにより複数のツールが一覧表示される場合があります。 Foundry ポータルでは、既定で新しいエージェントに **Web 検索**ツールが追加されるため、他のツールではなく、**Foundry IQ** ナレッジ ベース ツールの 3 つのドットを選択してください。
